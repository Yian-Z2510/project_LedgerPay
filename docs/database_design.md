...
| `payload` | `JSONB` | No | Complete immutable event payload snapshot |
| `status` | `VARCHAR(30)` | No | Default `PENDING` |
...
`payload` stores the complete JSON snapshot created at the time the business event occurs.

This ensures that:

- every retry sends the same event content;
- later changes to Payment or Order data do not change an existing event;
- the original event is auditable;
- delivery does not require rebuilding the payload through multiple queries.
