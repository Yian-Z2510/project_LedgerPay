...
handle_exit() {
  ...
  if [[ "$exit_status" -ne 0 && "$ROLLBACK_REQUIRED" == true && "$ROLLBACK_ATTEMPTED" == false ]]; then
    ...
    echo "Deployment exited unexpectedly; restoring the previous application images." >&2
    if rollback_application_images "$PREVIOUS_IMAGE_TAG"; then
...
main() {
  if [[ ! -f "$COMPOSE_FILE" || ! -f "$ENV_FILE" ]]; then
    echo "Production Compose or .env is missing in ${DEPLOY_DIR}." >&2
    return 1
  fi

  if [[ ! "$NEW_IMAGE_TAG" =~ ^[0-9a-f]{40}$ ]]; then
    echo "The deployment argument must be a full Git commit SHA." >&2
    return 1
  fi
  ...
  echo "Deployment health verification failed; rolling back application images." >&2
  ROLLBACK_ATTEMPTED=true
  if rollback_application_images "$PREVIOUS_IMAGE_TAG"; then
    echo "Rollback health verification passed; the deployment command will fail." >&2
  else
